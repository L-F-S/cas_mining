...
# Usage:
    ...
    # python 2fill_known__effector_cas_table.py <datasetname> <effector_cas_name>
         ...
         if ","+s3genome+"," in line:#this should only happen once in the whole file. we shall belive it is so.
            ...
            # print(uniref)
             ...
             for cas in prokkaslist:
                 if cas9ID in cas:
                     contig=cas.split("\t")[0]
                     unaltrostranosplit=cas.split("\t")
                     start=unaltrostranosplit[3]
                     stop=unaltrostranosplit[4]
                     f.close()
                     return contig, str(start)+" "+str(stop)
...
DF=pd.read_csv(outdir +"/"+s3dataset+"_known_"+feature+"_genomes.csv", index_col=0) #input e' come 3tabellazza, ma solo il subset con Cas9. piu piccolo e agile.
...
for index, genome in DF.iterrows(): 
    ...
    for record in SeqIO.parse(filename, "fasta"):
        ...
        if feature in record.description:
            #print("accade una volta per ", genomename, record.id)
            # devo comunque chiaramente mettere tutto qui
            contig, position=get_contig_from_tabellazza(genomename,working_dataset, record.id)
            ...
            outdf.at[newindexmitocca,"Contig"]=contig
            outdf.at[newindexmitocca, "Pos"]=position
            newindexmitocca+=1     
#
